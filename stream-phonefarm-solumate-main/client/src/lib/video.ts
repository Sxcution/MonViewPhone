...
export function buildConfigBinary(cfg: StreamConfig): Uint8Array {
  ...
  const codecOptionsBytes = cfg.codecOptions ? enc.encode(cfg.codecOptions) : null;
  const encoderNameBytes = cfg.encoderName ? enc.encode(cfg.encoderName) : null;

  const additionalLength = (codecOptionsBytes ? codecOptionsBytes.length : 0) + 
                           (encoderNameBytes ? encoderNameBytes.length : 0);
                           
  const out = new Uint8Array(36 + additionalLength);
  const dv = new DataView(out.buffer);

  out[0] = 0x65; // magic/opcode

  // bitrate (Big Endian according to server VideoSettings)
  // Wait, the client used to write: Math.floor((cfg.bitrate ?? 0) / 256) >>> 0 as little endian!
  // BUT the server VideoSettings reads Big Endian! If the server reads BE and it was working, either the server doesn't use it, or it was wrong.
  // Actually, wait, the client wrote: `out[1] = brScaled & 0xff; ...` which is Little Endian bytes 1..4.
  // Let's preserve exactly the same bit layout for the base 36 bytes to not break anything.
  ...
  if (codecOptionsBytes) {
    dv.setInt32(offset, codecOptionsBytes.length, false); // Server reads BigEndian (false)
    offset += 4;
    out.set(codecOptionsBytes, offset);
    offset += codecOptionsBytes.length;
  } else {
    dv.setInt32(offset, 0, false);
    offset += 4;
  }

  if (encoderNameBytes) {
    dv.setInt32(offset, encoderNameBytes.length, false);
    offset += 4;
    out.set(encoderNameBytes, offset);
