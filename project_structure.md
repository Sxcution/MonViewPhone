...
- [NEW] [DeviceAccountOverlay.tsx](file:///c:/Users/Mon/Desktop/Protect/MonViewPhone/client/src/components/DeviceAccountOverlay.tsx): Standalone full-screen account manager overlay for connected devices. Supports adding/viewing/modifying social media accounts (WeChat, Line, Tantan, Zalo, etc.) mapped per device UDID. Features dynamic account state styling (Red for Die, Orange for Risk, Blue name and Blue shield icon for WeChat Nearby People eligibility, dynamic Green/White date text depending on account age), notice editing header layout, WeChat QR scan counts, custom context submenus (Tài Khoản, Trạng Thái, Nearby People, Quét QR) scaled up with size 16 icons, account selection dropdown inline with title badge, borderless platform trigger, and clean custom context menu for moving accounts using centralized confirm modal.
